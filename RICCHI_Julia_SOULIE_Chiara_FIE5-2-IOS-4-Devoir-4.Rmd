...
accueil <- devoir3_data_accueil  # déjà chargé dans ton Rmd

# Extraire les timestamps d'arrivées réelles des patients à l'accueil
arrivees <- patients %>%
  filter(ACTIVITY_MACRO == "Accueil") %>%
  group_by(ID, PRIO) %>%
  summarise(heure_arrivee = min(DATETIME_BEGIN), .groups = "drop") %>%
  arrange(heure_arrivee) %>%
  mutate(
    # Convertir en minutes depuis 8h00 (début de journée)
    minutes_depuis_8h = as.numeric(difftime(heure_arrivee, 
                                             as.POSIXct("2015-11-12 08:00:00"), 
                                             units = "mins"))
  )

write.csv2(arrivees, "local_data/arrivees_accueil.csv", row.names = FALSE)

# Exporter aussi les paramètres gamma de l'accueil pour AnyLogic
...
arrivees <- arrivees %>%
  mutate(heure_arrivee_str = format(heure_arrivee, "%Y-%m-%d %H:%M:%S"))
arrivees <- arrivees %>%
