...
Comme AnyLogic utilise la paramétrisation `gamma(shape, scale)`, nous avons converti `rate` en `scale` par la relation `scale = 1/rate`.

La probabilité `0,868` utilisée dans le bloc `choixApresAccueil` correspond à la transition `Accueil → Consultation` estimée dans la matrice de transition de la chaîne de Markov.
Elle est obtenue en divisant le nombre de transitions observées de `Accueil` vers `Consultation` par le nombre total de transitions sortant de l’état `Accueil`.
